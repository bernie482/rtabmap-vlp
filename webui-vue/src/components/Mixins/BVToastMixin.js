...
const BVToastMixin = {
  ...
  methods: {
    ...
    $_skip_filter(message) {
      // Return true to skip toast.
      if (Error.CANCEL || Error.IGNORE) {
        let m = message?.toString();
        return m && (m.includes(Error.CANCEL) || m.includes(Error.IGNORE));
      }
      return false;
    },
    successToast(
