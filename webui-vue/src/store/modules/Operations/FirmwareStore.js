...
const FirmwareStore = {
  ...
  getters: {
    ...
    firmwareInventoryUrl: (state) => state.firmwareInventoryUrl,
  },
  mutations: {
    setFirmwareInventoryUrl: (state, url) => (state.firmwareInventoryUrl = url),
  ...
  actions: {
    ...
    getActiveBmcFirmware({ commit }) {
      return api
        ...
        .then(({ data: { Links } }) => {
          const id =
            Links?.ActiveSoftwareImage?.['@odata.id'].split('/').pop() ?? null;
    ...
    getUpdateServiceSettings({ commit }) {
      api
        ...
        .then(({ data }) => {
          ...
          const fwInventUrl = data?.FirmwareInventory?.['@odata.id'];

          commit('setApplyTime', applyTime);
          if (allowableActions?.includes('TFTP')) {
            commit('setTftpUploadAvailable', true);
          }
          commit('setFirmwareInventoryUrl', fwInventUrl);
        })
        .catch((error) => console.log(error));
    },
