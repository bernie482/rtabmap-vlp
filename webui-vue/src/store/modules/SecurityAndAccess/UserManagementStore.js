...
function assertSpecialError(error, options) {
  ...
  if (errmsg) {
    if (
      ...
      throw new Error(i18n.t('pageUserManagement.toast.errorPasswordFormat'));
    } else if (
      ...
      throw new Error(
        i18n.t('pageUserManagement.toast.errorUserExists', options)
      );
    }
  }
}
  ...
  actions: {
    ...
    async createUser({ dispatch }, { username, password, privilege, status }) {
      ...
      return await api
        ...
        .catch((error) => {
          console.log(error);
          assertSpecialError(error, { username });
