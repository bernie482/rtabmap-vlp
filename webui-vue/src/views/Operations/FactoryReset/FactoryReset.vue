          ...
          <b-form-group :label="$t('pageFactoryReset.form.resetOptionsLabel')">
            ...
            >
              ...
              <b-form-radio
                class="mb-1"
                value="resetNetwork"
                aria-describedby="reset-network"
                data-test-id="factoryReset-radio-resetNetwork"
              >
                {{ $t('pageFactoryReset.form.resetNetworkOptionLabel') }}
              </b-form-radio>
              <b-form-text id="reset-network" class="ml-4 mb-3">
                {{ $t('pageFactoryReset.form.resetNetworkOptionHelperText') }}
              </b-form-text>
            </b-form-radio-group>
          </b-form-group>
          <b-button
...
export default {
  ...
  methods: {
    ...
    onOkConfirm() {
      ...
      } else if (this.resetOption == 'resetToDefaults') {
        this.onResetToDefaultsConfirm();
      } else if (this.resetOption == 'resetNetwork') {
        this.onResetNetworkConfirm();
      }
    },
    onResetBiosConfirm() {
      this.$store
    ...
    onResetNetworkConfirm() {
      this.$store
        .dispatch('factoryReset/resetNetwork')
        .then((title) => {
          this.successToast('', {
            title,
          });
