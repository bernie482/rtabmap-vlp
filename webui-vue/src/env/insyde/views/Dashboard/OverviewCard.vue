<template>
  <b-card bg-variant="light" border-variant="light" class="mb-4">
    <div class="justify-content-between align-items-center d-flex flex-wrap">
      <b-link :to="to"
        ><h3 class="h5 mb-0">{{ title }}</h3></b-link
      >
      ...
      <!-- TODO: This method should be correct, but how to know each page is currently unknown -->
      <!-- <icon-restart
        class="pointer"
        variant="link"
        @click="onIconAction($event)"
      /> -->
    </div>
    <hr />
    <slot></slot>
...
export default {
  ...
  methods: {
    /*onIconAction(event) {
      console.log('onIconAction', event.target);
      console.log('onIconAction', event.currentTarget);
      console.log('onIconAction', event);
    },*/
  },
};
</script>
