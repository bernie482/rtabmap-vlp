<template>
  <div class="gauge">
    ...
    >
      ...
      <template v-if="scaleLines && needleLines.length == 0">
      ...
      <template v-if="needleLines">
        <!-- Display a line for each tick of the scale -->
        <line
          v-for="(line, index) in needleLines"
          :key="`${line.xE}-${index}`"
          :x1="line.xS"
          :y1="line.yS"
          :x2="line.xE"
...
<script>
// ref: https://www.npmjs.com/package/vue-svg-gauge
...
export default {
  ...
  computed: {
     ...
     */
    needleLines() {
      const { getAngle, innerRadius } = this;
      let lines = [];
      let lists = [this.value];
      if (this.values.length > 0) lists = this.values;
      for (let value of lists) {
        if (isNaN(value)) continue;

        const angle = getAngle(value);
        const length = 8;
        const startCoordinate = polarToCartesian(innerRadius, angle);
        const endCoordinate = polarToCartesian(innerRadius - length, angle);
  ...
  watch: {
    ...
    value: {
      ...
      handler(next) {
        ...
        // PATCH: input value may not be a number, show min value
        next = isNaN(next) ? this.min : next;
        let safeValue = next;
