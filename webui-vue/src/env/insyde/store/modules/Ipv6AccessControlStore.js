  ...
  actions: {
    ...
    async getRules({ commit }) {
      ...
      return await api
        ...
        .then((response) => {
          let enabled = response.data.ipv6?.config?.enable || false;
          const data = response.data.ipv6?.rules?.map((rule) => {
            return {
              ruleNo: rule.index,
              ruleType: rule.type,
...
export default Ipv6AccessControlStore;
